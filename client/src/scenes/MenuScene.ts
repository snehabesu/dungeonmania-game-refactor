...
class MenuScene extends Scene {
  ...
  create() {
    ...
    // GENERATE DUNGEON
    this.genGame = new TextButton(
      this,
      10,
      this.renderer.height / 4 + 
        this.newGame.height + 75 + 10,
      ...
      async () => {
        ...
        const result = await Swal.fire({
          ...
          heightAuto: false,
          html: `
          <label for="xstart" class="f6 b db mb2 mt3">xStart<span class="normal black-60"></span></label>
          <input id="xstart" class="swal2-input" required="" type="number"/>
          
          <label for="ystart" class="f6 b db mb2 mt3">yStart<span class="normal black-60"></span></label>
          <input id="ystart" class="swal2-input" required="" type="number"/>
          
          <label for="xend" class="f6 b db mb2 mt3">xEnd<span class="normal black-60"></span></label>
          <input id="xend" class="swal2-input" required="" type="number"/>
          
          <label for="yend" class="f6 b db mb2 mt3">yEnd<span class="normal black-60"></span></label>
          <input id="yend" class="swal2-input" required="" type="number"/>
          
          <label for="config" class="f6 b db mb2 mt3">Configuration<span class="normal black-60"></span></label>
          <select id="config" class="swal2-input" list="configs" required="">
            <datalist id="configs">
              ${configs.map((x) => `<option>${x}</option>`).join("\n")}
            </datalist>
          </select>
          ...
          preConfirm: async () => {
            ...
            if (await inputValidator(results)) {
              dungeon = await API.generate(Number(results[0]), Number(results[1]), Number(results[2]), Number(results[3]), results[4]);
              return !!dungeon;
            }
          },
        });
    ...
    this.add.existing(this.genGame);
    this.genGame.setX(-this.genGame.width);
    ...
    this.tweens.add({
      delay: 3500,
      duration: 1000,
      targets: [this.newGame, this.genGame, this.loadGame, this.credits, this.exitGame],
      x: 10,
      ease: "Bounce",
    });
