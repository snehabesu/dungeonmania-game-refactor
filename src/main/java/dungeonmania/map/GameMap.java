...
public class GameMap {
    ...
    public void moveTo(Entity entity, Direction direction) {
        Position finalPos = Position.translateBy(entity.getPosition(), direction);
    ...
    public boolean canMoveTo(Entity entity, Position position) {
        return !nodes.containsKey(position) || nodes.get(position).canMoveOnto(this, entity); //demeter
    }
    ...
    public void updateWires() {
        List<Wire> allWires = getEntities(Wire.class);
        for (Wire wire : allWires) {
            wire.deactivate();
        }
        List<Switch> allSwitches = getEntities(Switch.class);
        List<Switch> activatedSwitches = new ArrayList<>();
        for (Switch switches : allSwitches) {
            if (switches.isActivated()) {
                activatedSwitches.add(switches);
            }
        }
        activatedSwitches.sort(Comparator.comparingInt(Switch :: getActivatedTick).reversed());
        for (Switch switches : activatedSwitches) {
            switches.activateWires(this);
        }
    }

    public void updateLogicals() {
        List<SwitchDoor> allSwitchDoors = getEntities(SwitchDoor.class);
        for (SwitchDoor switchDoor : allSwitchDoors) {
            switchDoor.updateActivation(this);
        }
        List<LightBulb> allLightBulbs = getEntities(LightBulb.class);
        for (LightBulb lightBulb : allLightBulbs) {
            lightBulb.updateActivation(this);
        }
    }
