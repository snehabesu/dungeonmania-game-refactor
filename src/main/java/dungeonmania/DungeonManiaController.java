...
public class DungeonManiaController {
    ...
    public DungeonResponse generateDungeon(
        ...
        if (xEnd < xStart || yEnd < yStart) {
            throw new IllegalArgumentException("Invalid dungeon dimensions");
        }

        try {
            GameBuilder builder = new GameBuilder();
            game = builder.setConfigName(configName).setDungeonName(UUID.randomUUID().toString())
                .genRandomGame(xStart, yStart, xEnd, yEnd);
            return ResponseBuilder.getDungeonResponse(game);
        } catch (JSONException e) {
            return null;
        }
